...
<template>
    ...
    <div v-if="data?.themes != undefined" :class="`${data?.site_lang} page_${$store.segments.join('_')}`">
        ...
        <Header :cartData="cartData" :wishlistData="wishlistData" :userData="userData" :apiData="apiData"
            :globalData="globalData">
        </Header>
        <div>
            <template v-for="(item) in data?.themes">
                <p v-if="development" class="bg-dark fw-bold text-white py-2 text-center">
                    {{ item.theme }}
                </p>
                <component :is="`Theme${item.theme}`" :content="item?.value" :cartData="cartData"
                    :wishlistData="wishlistData" :apiData="apiData" :userData="userData" v-model:apiData="apiData"
                    :globalData="globalData" />
            </template>
        </div>
        <Footer :cartData="cartData" :wishlistData="wishlistData" :userData="userData" :apiData="apiData"
            :globalData="globalData"></Footer>
        <div style="width: 100%; height: 32px; border: 0; outline: 0; position: fixed; z-index: 9999; bottom: 0"
            v-if="apiData.admin_iframe != undefined && apiData.admin_iframe != ''">
