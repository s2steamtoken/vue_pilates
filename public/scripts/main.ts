...
const MEDIA_ARRAY: number[] = [0, 500, 768, 1024, 1440, 1920];
let HTML_FONT_SIZE: number = 16;
...
function change_font_size_variable(
	media_start: number,
	media_next: number,
	font_size: number = 16
): number {
	const result: number = (media_next / media_start) * font_size;
	return result;
}
function check_media(media_array: number[]): number {
	let result: number = 0;
...
function calc_adaptation(font_size: number, media: number) {
	let result = font_size * (window.innerWidth / media);

	return result;
}

function change_font_size(font_size: number, media_array: number[]): void {
	let media_font_size: number = (document.body.offsetWidth <= 500) ? change_font_size_variable(375, 500, font_size) : font_size;
